...
app.post('/get-users', (req, res) => {
  ...
  connection.query('select username from users where pharmacy_name = (select u.pharmacy_name from users u where u.username = ? ) and role <> 1 and username like ?',[session.user.username,searchPattern], (err, result, fields) => {
    if (err) {
      res.status(200).send({
        status: "error",
        message: "Something went wrong"
